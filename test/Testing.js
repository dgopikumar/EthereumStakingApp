...
//const { stack } = require("fp-ts/lib/Exception");
...
describe('Staking', function() {
    ...
    describe('stakeEther', function() {
        it('transfer ether', async function() {
            ...
            // test the change in signer1 ethers balance
    ...
    describe('modifyLockPeriods', function() {
        ...
        describe('owner', function() {
            it('should create a new lock period', async function() {
                ...
                expect(await staking.tiers(100)).to.equal(1100);
                expect(await staking.lockPeriods(3)).to.equal(100);
            });

            it('should modify an existing lock period', async function() {
                await staking.connect(signer1).modifyLockPeriods(30, 7500);
                expect(await staking.tiers(30)).to.equal(7500);
            });
        });

        describe('non-owner', async function() {
            it('reverts', async function() {
                expect(staking.connect(signer2).modifyLockPeriods(100, 1100)).to.be.revertedWith('Only owner may modify staking periods');
            });
        });
    });
    ...
    describe('getPositionById', function() {
        it('returns data about a specific position, given a positionId', async () => {
            ...
            const block = await provider.getBlock(receipt.blockNumer);
    ...
    describe('getPositoinIdsForAddress', function() {
        it('returns a list of positionsIds created by a specidic address', async () => {
            let data;
            let transaction;
    ...
    describe('changeUnlockDate', function() {
        ...
        describe('non-owner', function() {
            it('reverts', async () => {
                ...
                expect(staking.connect(signer2).changeUnlockDate(0, newUnlockDate)).to.be.revertedWith('Only owner may modify staking periods')
    ...
    //             transction = await staking.connect(signer2).closePosition(0)
    ...
    //             const balanceBefore = await signer2.getBalance();
    //             const position = await staking.getPositionById(0);
    //             transaction = await staking.connect(signer2).closePosition(0);
    //             receipt = await transaction.wait();

    //             const gasUsed = receipt.gasUsed.mul(receipt.effectiveGasPrice);
    //             const balanceAfter = await signer2.getBalance();
